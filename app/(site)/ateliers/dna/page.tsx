...
export const metadata: Metadata = {
  title: "Atelier DNA",
  description: "Un atelier dynamique et créatif pour découvrir la danse par le mouvement, le rythme et le plaisir d’être ensemble.",
};
    ...
    <>
      <section className="relative isolate flex min-h-svh flex-col justify-end">
        <div className="absolute inset-0 overflow-hidden [clip-path:inset(0)]">
          ...
          <Image src="https://images.unsplash.com/photo-1547153760-18fc86324498?w=1800&h=1200&fit=crop&q=80" alt="" fill priority sizes="100vw" className="dna-drift object-cover grayscale-[.4] brightness-[.52] contrast-[1.08] will-change-transform" />
      ...
      <section className="overflow-hidden pb-[clamp(80px,12vh,140px)] pt-[clamp(20px,4vh,50px)]">
        <div className="dna-marquee-slow flex w-max gap-0.5 will-change-transform">
          {[...photos, ...photos].map((photo, index) => <div key={`${photo}-${index}`} className="relative aspect-[3/4] w-[clamp(200px,22vw,340px)] overflow-hidden bg-[#0d0d0d]"><Image src={imageUrl(photo)} alt="" fill sizes="22vw" className="object-cover grayscale-[.55] brightness-[.82]" /></div>)}
        </div>
        <div className="dna-marquee-reverse mt-0.5 flex w-max gap-0.5 will-change-transform">
          {[...photos2, ...photos2].map((photo, index) => <div key={`${photo}-${index}`} className="relative aspect-[4/3] w-[clamp(160px,17vw,260px)] overflow-hidden bg-[#0d0d0d]"><Image src={imageUrl(photo, "w=600&h=450")} alt="" fill sizes="17vw" className="object-cover grayscale-[.7] brightness-[.7]" /></div>)}
        </div>
      </section>
      ...
      <section className="py-[clamp(80px,12vh,150px)]">
        <div className="site-container">
          ...
          <div className="grid grid-cols-[repeat(auto-fit,minmax(min(100%,240px),1fr))] gap-[clamp(16px,2.4vw,36px)]">{ages.map((age) => <article key={age.title} data-reveal className="group"><div className="relative mb-[22px] aspect-[4/5] overflow-hidden bg-[#0d0d0d]"><Image src={imageUrl(age.image, "w=800&h=1000")} alt="" fill sizes="(max-width: 768px) 100vw, 33vw" className="object-cover grayscale-[.85] brightness-[.72] transition duration-700 ease-[cubic-bezier(.16,1,.3,1)] group-hover:scale-[1.06] group-hover:grayscale-0 group-hover:brightness-95" /></div><h3 className="mb-2.5 font-display text-[clamp(21px,1.8vw,28px)]">{age.title}</h3><p className="max-w-[32ch] text-[14.5px] leading-[1.7] text-[#AAA6A3]">{age.description}</p></article>)}</div>
        </div>
      </section>
      ...
      <section className="relative flex min-h-[78svh] items-center overflow-hidden">
        <div className="absolute inset-0 overflow-hidden"><div className="absolute inset-0 bg-[repeating-linear-gradient(45deg,#131313_0_12px,#0b0b0b_12px_24px)]" /><div data-parallax="0.12" className="absolute -inset-[8%]"><Image src="https://images.unsplash.com/photo-1535525153412-5a42439a210d?w=1800&h=1100&fit=crop&q=80" alt="" fill sizes="100vw" className="object-cover grayscale-[.5] brightness-[.42]" /></div><div className="dna-halo absolute inset-0 bg-[radial-gradient(46%_58%_at_50%_52%,rgba(239,47,41,.3),transparent_70%)]" /><div className="absolute inset-0 bg-[linear-gradient(180deg,rgba(5,5,5,.7),rgba(5,5,5,.45)_45%,rgba(5,5,5,.85))]" /></div>
        <div className="relative z-[2] mx-auto w-full max-w-[1000px] px-[clamp(20px,4vw,60px)] py-[clamp(60px,9vh,110px)] text-center"><p data-reveal className="mb-6 text-[clamp(15px,1.1vw,17px)] leading-[1.75] text-[#AAA6A3]">Un moment de partage, d’énergie et de plaisir à travers la danse.</p><h2 data-reveal style={{ "--reveal-delay": "100ms" } as CSSProperties} className="mb-[38px] font-display text-[clamp(42px,7vw,120px)] leading-[.95] tracking-[-.025em]">Venez <em className="font-normal text-[#EF2F29]">danser</em></h2><div data-reveal style={{ "--reveal-delay": "200ms" } as CSSProperties} className="flex flex-wrap justify-center gap-3.5"><Link href="/#contact" className="bg-[#EF2F29] px-8 py-[17px] text-xs uppercase tracking-[.16em] text-[#050505] transition hover:-translate-y-[3px] hover:shadow-[0_20px_48px_rgba(239,47,41,.35)]">Nous contacter</Link><Link href="/#ateliers" className="border border-[rgba(243,239,233,.24)] px-8 py-[17px] text-xs uppercase tracking-[.16em] transition hover:-translate-y-[3px] hover:border-[#F3EFE9]">Les autres ateliers</Link></div></div>
      </section>
    </>
  );
}
